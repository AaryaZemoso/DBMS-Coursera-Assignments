...
drop database `DBMSCoursera`;
create database `DBMSCoursera`;
...
create table `LOCATION`(
    ...
    constraint `LOCATION_FACILITY_FK` foreign key (FACNO) references `FACILITY`(FACNO)
...
create table `EVENTREQUEST`(
    ...
    ESTAUDIENCE float not null,
    
    BUDNO varchar(5),
    
    constraint `EVENTREQUEST_PK` primary key(EVENTNO),
    constraint `EVENTREQUEST_CUSTOMER_FK` foreign key (CUSTNO) references `CUSTOMER`(CUSTNO),
    constraint `EVENTREQUEST_FACILITY_FK` foreign key (FACNO) references `FACILITY` (FACNO)
...
create table `EVENTPLAN`(
    ...
    constraint `EVENTPLAN_EVENTREQUEST_FK` foreign key (EVENTNO) references `EVENTREQUEST`(EVENTNO),
    constraint `EVENTPLAN_EMPLOYEE_FK` foreign key (EMPNO) references `EMPLOYEE`(EMPNO)
...
create table `EVENTPLANLINE`(
    ...
    constraint `EVENTPLANLINE_EVENTPLAN_FK` foreign key (PLANNO) references `EVENTPLAN`(PLANNO),
    constraint `EVENTPLANLINE_LOCATION_FK` foreign key (LOCNO) references `LOCATION`(LOCNO),
    constraint `EVENTPLANLINE_RESOURCETBL_FK` foreign key (RESNO) references `RESOURCETBL`(RESNO)
